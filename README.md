...
It is also possible to remove a bond by calling `removeBond`. Note that this method uses a hidden Android API and may stop working in the future. When calling the `removeBand` method, the peripheral will also disappear from the settings menu on the phone.
